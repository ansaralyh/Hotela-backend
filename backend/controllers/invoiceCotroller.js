...
exports.index = catchAsyncError(async (req, res, next) => {
  ...
  const invoices = await Invoice.find({ hotel_id: req.user.id, branch_id: id }).populate("customer_id").populate({ path: "reservation_id", populate: { path: "room_id", populate:{path:"room_category"} } })
    .skip(startIndex)
    .limit(limit);
